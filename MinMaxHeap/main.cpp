...
int main(int argc, char* argv[]){
	...
	for(int s = 0; s < 4; s++){
		...
		for(int i = 1; i<= 5; i++){
			//-------------------------------BUILD TESTS--------------------
			MinMaxHeap *my_mmh = new MinMaxHeap();
			srand(i); // give rand() a seed
			my_timer.start();
			for(int k = 1; k <= n; k++){
				// randomly generate a number from 1 to 4*test_set[i]
				generated = rand() % (4*n) + 1;
				my_mmh->constructive_insert(generated);
			}
			my_mmh->heapify();
			duration = my_timer.stop();
			write << "MinMaxHeap build duration #" << i << ": " << duration << " seconds\n";
			...
			for(int k = 1; k <= n; k++){
				// randomly generate a number from 1 to 4*test_set[i]
				generated = rand() % (4*n) + 1;
				my_heap->constructive_insert(generated);
			}
			my_heap->heapify();
			duration = my_timer.stop();
			...
			operation(n, my_mmh);
			duration = my_timer.stop();
			write << "MinMaxHeap operation duration #" << i << ": " << duration << " seconds\n";
			srand(i);
			my_timer.start();
			operation(n, my_heap);
			duration = my_timer.stop();
			...
			delete my_mmh; my_mmh = nullptr;
			delete my_heap; my_heap = nullptr;
